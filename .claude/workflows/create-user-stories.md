---
description: Créer et structurer des User Stories. Se déclenche avec /create-user-stories ou quand on découpe une feature en tâches numérotées.
---
...
1. **Collecte des exigences spécifiques** : Demandez à l'utilisateur de fournir les informations concernant la User Story à en utilisant le format suivant :
   ...
   **5. Scénarios de Test / Preuves de QA (OBLIGATOIRE) :**
   - [Ex: Appel curl POST avec payload valide -> Retour 201 + Entrée DB vérifiée]
   - [Ex: Appel avec MatchID inexistant -> Retour 404]
   - [Ex: Vérification visuelle du composant X sur mobile]
   ```
...
## Notes
- Enregistrez chaque User Story dans un fichier `.md` nommé selon le modèle `US-<numéro>-<rôle>-<nom-court>.md` (par exemple `US-150-front-inscription.md`) et placez ce fichier dans le dossier correspondant à la fonctionnalité : `docs/features/Vxx-[FeatureName]/`.
