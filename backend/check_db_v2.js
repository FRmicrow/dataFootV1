...
async function testFetch() {
    ...
    if (fixtures.length > 0) {
        ...
        const info = db.get("SELECT f.fixture_id, f.match_date, l.name as league FROM V3_Fixtures f JOIN V3_Leagues l ON f.league_id = l.league_id WHERE f.fixture_id = ?", [fid]);
        console.log(info);
    }
}
