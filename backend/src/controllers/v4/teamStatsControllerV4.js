...
export const getTypicalLineup = async (req, res) => {
    try {
        const teamId = req.params.id; // Usually a team_id or slug from V4
        ...
        if (isNaN(teamId)) {
            const team = await db.get(`SELECT team_id FROM v4.teams WHERE slug = ?`, [teamId]);
            if (team) resolvedTeamId = team.team_id;
        }
        ...
        let matchQuery = `
            SELECT 
                CASE 
                    WHEN m.home_team_id = ? THEN m.home_formation 
        ...
        matchQuery += ` 
            AND CASE WHEN m.home_team_id = ? THEN m.home_formation ELSE m.away_formation END IS NOT NULL
            GROUP BY formation 
        ...
        const totalMatchesReq = await db.get(
            `SELECT COUNT(*) as c FROM v4.matches WHERE (home_team_id = ? OR away_team_id = ?) AND season_label = ?` + 
        ...
        const totalWinsReq = await db.get(
            ...
            AND CASE WHEN home_team_id = ? THEN home_formation ELSE away_formation END = ?` +
            (competition ? ` AND competition_id = ?::BIGINT` : ''),
        ...
        const rosterQuery = `
            ...
            AND CASE WHEN m.home_team_id = ? THEN m.home_formation ELSE m.away_formation END = ?
...
export const getTeamTacticalSummary = async (req, res) => {
    try {
        ...
        if (history === 'true') {
            ...
            const historyQuery = `
                SELECT 
                    ...
                    SUM(CASE WHEN (m.home_team_id = ? AND m.home_score > m.away_score) OR (m.away_team_id = ? AND m.away_score > m.home_score) THEN 1 ELSE 0 END)::int as wins,
                    SUM(CASE WHEN m.home_score = m.away_score THEN 1 ELSE 0 END)::int as draws,
                    SUM(CASE WHEN (m.home_team_id = ? AND m.home_score < m.away_score) OR (m.away_team_id = ? AND m.away_score < m.home_score) THEN 1 ELSE 0 END)::int as losses,
                    SUM(CASE WHEN m.home_team_id = ? THEN m.home_score ELSE m.away_score END)::int as goals_scored,
                    SUM(CASE WHEN m.home_team_id = ? THEN m.away_score ELSE m.home_score END)::int as goals_conceded,
                    SUM(CASE WHEN (m.home_team_id = ? AND m.away_score = 0) OR (m.away_team_id = ? AND m.home_score = 0) THEN 1 ELSE 0 END)::int as clean_sheets,
                    -- Stats across matches 
                    AVG(CASE WHEN m.home_team_id = ? THEN ms.home_poss_ft ELSE ms.away_poss_ft END) as possession,
            ...
            const overviewQuery = `
                WITH match_data AS (
                    SELECT 
                        ...
                        CASE WHEN m.home_team_id = ? THEN m.home_score ELSE m.away_score END as goals_scored,
                        CASE WHEN m.home_team_id = ? THEN m.away_score ELSE m.home_score END as goals_conceded,
                        CASE WHEN m.home_team_id = ? THEN ms.home_poss_ft ELSE ms.away_poss_ft END as possession,
                        0 as pass_accuracy,
                        CASE WHEN m.home_team_id = ? THEN ms.home_corners_ft ELSE ms.away_corners_ft END as corners,
                        CASE WHEN m.home_team_id = ? THEN ms.home_shots_ft ELSE ms.away_shots_ft END as total_shots,
                        CASE WHEN m.home_team_id = ? THEN ms.home_shots_ot_ft ELSE ms.away_shots_ot_ft END as shots_on_target,
                        0 as saves,
                        CASE WHEN m.home_team_id = ? THEN ms.home_yellows_ft ELSE ms.away_yellows_ft END as yellow_cards
                    FROM v4.matches m
                    LEFT JOIN v4.match_stats ms ON ms.match_id = m.match_id
                    WHERE (m.home_team_id = ? OR m.away_team_id = ?)
                    AND m.season_label = ?
            ...
            let xgQuery = `SELECT xg, xga, npxg, npxga FROM v4.team_season_xg WHERE team_id = ? AND season_label = ?`;
            if (competition) {
        ...
        logger.error({ err: error }, "Error in getTeamTacticalSummary V4");
        res.status(500).json({ success: false, error: "Internal server error" });
    }
};
