export const up = async (db) => {
    ...
    await db.run(`CREATE TABLE IF NOT EXISTS V3_Fixture_Lineup_Players (
    ...
    await db.run(`CREATE TABLE IF NOT EXISTS V3_Team_Features_PreMatch (
        fixture_id INTEGER NOT NULL,
        team_id INTEGER NOT NULL,
    ...
    await db.run(`CREATE TABLE IF NOT EXISTS V3_Submodel_Outputs (
        fixture_id INTEGER NOT NULL,
        team_id INTEGER NOT NULL,
    ...
    await db.run(`CREATE TABLE IF NOT EXISTS V3_ML_Feature_Store_V2 (
        fixture_id INTEGER NOT NULL,
        league_id INTEGER NOT NULL,
    ...
    await db.run(`CREATE TABLE IF NOT EXISTS V3_Model_Registry (
    ...
    await db.run(`CREATE TABLE IF NOT EXISTS V3_Training_Runs (
        id SERIAL PRIMARY KEY,
    ...
    await db.run(`CREATE TABLE IF NOT EXISTS V3_ML_Predictions (
        id SERIAL PRIMARY KEY,
        fixture_id INTEGER NOT NULL,
        model_registry_id INTEGER,
        feature_set_id TEXT,
    ...
    try {
        await db.run("ALTER TABLE V3_Fixture_Stats ADD COLUMN ball_possession_pct INTEGER");
    } catch (e) {
        if (!e.message.includes('already exists')) throw e;
    }

    // 9. Indexes
    await db.run('CREATE INDEX IF NOT EXISTS idx_ml_features_prematch_team ON V3_Team_Features_PreMatch(team_id)');
    await db.run('CREATE INDEX IF NOT EXISTS idx_ml_predictions_fixture ON V3_ML_Predictions(fixture_id)');
    await db.run('CREATE INDEX IF NOT EXISTS idx_v3_fixture_lineup_players_team ON V3_Fixture_Lineup_Players(team_id)');
};
