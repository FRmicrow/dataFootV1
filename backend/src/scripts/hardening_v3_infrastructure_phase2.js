...
const enforceApiIdIntegrity = (db) => {
...
const findTrophyMatch = (leagueMap, tLeague, tCountry) => {
    // Strategy A: Exact Match
    let match = leagueMap.find(l => l.name === tLeague);
    // Strategy B: Disambiguated Match
    if (!match) match = leagueMap.find(l => l.name === `${tLeague} (${tCountry})`);
    // Strategy C: Partial Match
    if (!match) match = leagueMap.find(l => l.name.startsWith(tLeague) && l.country === tCountry);
    return match;
};

const advancedTrophyMapping = (db) => {
    console.log('\n--- 2. Advanced Trophy Normalization (Disambiguation) ---');
    const leagueMap = [];
    const leaguesRes = db.exec(`SELECT l.league_id, l.name, c.name as country_name FROM V3_Leagues l JOIN V3_Countries c ON l.country_id = c.country_id`);
    ...
    if (unmappedRes.length > 0) {
        ...
        for (const [id, tLeague, tCountry] of rows) {
            const match = findTrophyMatch(leagueMap, tLeague, tCountry);
            if (match) {
                db.run(`UPDATE V3_Trophies SET competition_id = ?, country = ? WHERE id = ?`, [match.id, match.country, id]);
                mappedCount++;
            }
        }
        console.log(`✅ successfully mapped ${mappedCount} additional trophies.`);
    }
};
...
const finalSweep = (db) => {
    ...
    db.run(`DELETE FROM V3_Trophies WHERE id NOT IN (SELECT MAX(id) FROM V3_Trophies GROUP BY player_id, IFNULL(competition_id, league_name), trophy, season, place)`);
...
async function run() {
    ...
    enforceApiIdIntegrity(db);
    advancedTrophyMapping(db);
    finalSweep(db);
