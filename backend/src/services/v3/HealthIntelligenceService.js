...
export class HealthIntelligenceService {
    static async calculateScore() {
        ...
        const missingFixturesLeagues = (await db.get(`
            SELECT COUNT(*) as count 
            FROM V3_League_Seasons
            WHERE imported_players = true AND (imported_fixtures = false OR imported_fixtures IS NULL)
        `)).count;

        score -= (missingFixturesLeagues * 5);

        // 4. Elite Coverage Bonus (+10 if all Top 10 high-prestige countries have 100% sync)
        const eliteGaps = (await db.get(`
            SELECT COUNT(*) as count FROM (
                ...
                HAVING MIN(CAST(ls.imported_players AS integer)) = 0
            ) AS prestige_gaps
        `)).count;
        ...
        const totals = await db.get(`
            SELECT 
                ...
                SUM(CASE WHEN imported_players = true AND imported_events = true AND imported_lineups = true THEN 1 ELSE 0 END) as fully_synced
            FROM V3_League_Seasons
        `);
