...
class ClubServiceV4 {
    ...
    async getClubMatches(clubId, options = {}) {
        ...
        let query = `
            ...
            JOIN v4.competitions comp ON m.competition_id = comp.competition_id
            LEFT JOIN latest_club_logos hcl ON hc.club_id = hcl.club_id
            LEFT JOIN latest_club_logos acl ON ac.club_id = acl.club_id
            WHERE (m.home_club_id = ? OR m.away_club_id = ?)
        `;
    ...
    async getClubSummary(clubId, seasonLabel, competitionId = null) {
        let query = `
            SELECT 
                ...
                SUM(CASE WHEN (m.home_club_id = ? AND m.home_score > m.away_score) OR (m.away_club_id = ? AND m.away_score > m.home_score) THEN 1 ELSE 0 END) as total_wins,
                SUM(CASE WHEN m.home_club_id = ? THEN m.home_score ELSE m.away_score END) as goals_scored,
                SUM(CASE WHEN m.home_club_id = ? THEN m.away_score ELSE m.home_score END) as goals_conceded,
