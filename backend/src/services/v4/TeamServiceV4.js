...
class TeamServiceV4 {
    async getTeamProfile(identifier) {
        return db.get(
            `WITH ${LATEST_CLUB_LOGOS_CTE}
             ...
             FROM v4.teams c
             LEFT JOIN latest_team_logos lcl ON c.team_id = lcl.team_id
    ...
    async getTeamSquad(teamId, seasonLabel, competitionId = null) {
        let query = `
            WITH player_stats AS (
                ...
                WHERE ml.team_id = ? AND m.season_label = ?
        `;
        const params = [teamId, seasonLabel];
    ...
    async getTeamMatches(teamId, options = {}) {
        ...
        let query = `
            ...
            JOIN v4.competitions comp ON m.competition_id = comp.competition_id
            LEFT JOIN latest_team_logos hcl ON hc.team_id = hcl.team_id
            LEFT JOIN latest_team_logos acl ON ac.team_id = acl.team_id
            WHERE (m.home_team_id = ? OR m.away_team_id = ?)
        `;
    ...
    async getTeamSummary(teamId, seasonLabel, competitionId = null) {
        let query = `
            SELECT 
            ...
            WHERE (m.home_team_id = ? OR m.away_team_id = ?)
              ...
              AND m.season_label = ?
        `;
        const params = [teamId, teamId, teamId, teamId, teamId, teamId, seasonLabel];
...
export default new TeamServiceV4();
