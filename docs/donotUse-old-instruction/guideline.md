...
**Containers Over Local**: Always run the application using Docker (`docker compose up -d`) to ensure environment consistency.

**Leverage Hot Reloading**: Use the provided bind mounts for development. Do NOT run `npm run build` to test UI or API changes. Verify work directly via the dev servers (e.g., `http://localhost:5173`).
