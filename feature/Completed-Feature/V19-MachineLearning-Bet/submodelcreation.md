...
* PROCESS_V1 1H proxies (rolling 1H shots/corners/sot_rate_1h5)et FT process (shots/corners/discipline/control) comme contexte.
Dataset row
...
Objectif
Prédire λ_corners_total (corners totaux match FT).Puis en déduire P(Over X.5) pour plusieurs lignes.
Label
...
Label
* cards_total = yellow_home + yellow_away + 2*(red_home + red_away) (option)ou juste jaunes+rouges en brut selon ton usage.Données via V3_Fixture_Stats FT.
Features
...
* ordre des features
* noms
* typesdans un fichier feature_schema_META_V1.json
...
1. Walk-forward metrics stockées dans V3_Model_Registry.metrics_json
2. Artifacts modèles stockés sur disque (path enregistré si tu ajoutes un champ)
