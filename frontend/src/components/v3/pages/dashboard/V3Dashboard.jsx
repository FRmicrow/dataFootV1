...
const V3Dashboard = () => {
    ...
    if (loading || !stats) return (
        <PageLayout className="animate-fade-in">
            <Skeleton width="220px" height="42px" style={{ marginBottom: '8px' }} />
            <Skeleton width="340px" height="18px" style={{ marginBottom: 'var(--spacing-xl)' }} />
        ...
        <PageLayout className="animate-fade-in">
            <PageHeader
                ...
                extra={
                    <Stack direction="row" gap="var(--spacing-lg)" className="v3-header__progress-row">
                        <Progress label="Data Coverage" value={health_summary.coverage_percent} showLabel />
                        <Progress label="System Health" value={health_summary.score} variant={getHealthVariant(health_summary.score)} showLabel />
                    </Stack>
                }
            />
            ...
            <PageContent>
                ...
                <Grid columns="repeat(auto-fit, minmax(240px, 1fr))" gap="var(--spacing-md)" className="mb-xl">
                    ...
                    <MetricCard label="Players"  value={volumetrics.total_players.toLocaleString()}  subValue="Verified profiles" variant="featured" />
                ...
                <Grid columns="repeat(2, 1fr)" gap="var(--spacing-lg)" className="mb-xl">
                        ...
                        <div className="v3-chart-container">
                            <ResponsiveContainer>
                                <BarChart data={players_by_country} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
                                    ...
                                    <Tooltip contentStyle={{ background: 'var(--color-bg-card)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', backdropFilter: 'var(--glass-blur)' }} itemStyle={{ color: '#fff' }} cursor={{ fill: 'rgba(255,255,255,0.03)' }} />
                    ...
                    <Card title="Continental Scope" subtitle="Global distribution of scouted entities">
                        <div className="v3-chart-container">
                            <ResponsiveContainer>
                                <PieChart>
                                    <Pie data={distribution} dataKey="count" nameKey="continent" cx="50%" cy="45%" innerRadius={70} outerRadius={90} paddingAngle={8}>
                                        {distribution?.map((entry, i) => (
                                            <Cell key={`cell-${entry.continent}`} fill={COLORS[i % COLORS.length]} />
                                        ))}
                                    </Pie>
                                    <Tooltip contentStyle={{ background: 'var(--color-bg-card)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)' }} />
