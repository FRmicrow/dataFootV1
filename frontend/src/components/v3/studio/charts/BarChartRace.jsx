...
const BarChartRace = ({ data, width, height, isPlaying, onFrame, onComplete, speed = 1, className = "", title = "Chart Evolution", barCount = 10 }) => {
    ...
    // Layout Config - HD Scaled
    // Values are relative to 1080p roughly, but dynamic margin is better
    ...
    useEffect(() => {
        ...
        data.forEach(frame => {
            frame.records.forEach(r => uniquePlayers.add(JSON.stringify({
                id: r.id,
                url: r.image,
        ...
        uniquePlayers.forEach(json => {
            ...
            // 2. Load Team Logo (for drawing) & Extract Color
            // We use teamColorsRef to store both the color AND the image
    ...
    useEffect(() => {
        ...
        canvas.style.width = width + 'px'; // Ensure CSS matches logical size intent for scaling
        ...
        const draw = (timestamp) => {
            ...
            // Time Update
            if (isPlaying) {
                const duration = 2000 / speed; // 2 seconds per year at 1x
        ...
        const renderFrame = (t) => {
            ...
            // 3. Draw Watermark Year
            context.save();
            context.fillStyle = '#ffffff';
            context.globalAlpha = 0.08;
            // responsive font size: min(width, height) is not enough for vertical.
            // We need to ensure 4 digits fit within width.
            // Approx char width is 0.6em. 4 chars = 2.4em.
            // So fontSize * 2.4 < width. => fontSize < width / 2.4.
            ...
            // Shift yScale range to start AFTER the top axis labels
            // Add padding of 1 bar height
            const yScale = d3.scaleLinear()
                .domain([1, barCount])
                .range([margin.top + barHeight, height - margin.bottom]);

            // 5. Grid lines (Vertical) & Axis
            context.strokeStyle = 'rgba(255,255,255,0.05)';
            context.lineWidth = 2;
            context.beginPath();
            xScale.ticks(5).forEach(tick => {
                const x = margin.left + xScale(tick);
                context.moveTo(x, margin.top); // Grid line starts at labels
                context.lineTo(x, height - margin.bottom);

                // Axis Label (Horizontal Data)
                context.fillStyle = '#888';
                context.font = `600 ${height * 0.018}px Inter`;
                context.textAlign = 'center';
                context.textBaseline = 'bottom';
                // Draw label cleanly above the grid line
            ...
            visible.forEach(p => {
                ...
                const barW = Math.max(0, xScale(p.value)); // Avoid negative width
                ...
                let color = teamData?.color;
                if (!color) {
                    const hue = (p.id * 137.5) % 360;
                    color = `hsl(${hue}, 70%, 55%)`; // Fallback
                }
                ...
                context.beginPath();
                // Bar starts at margin.left
                context.roundRect(margin.left, barY - barHeight / 2, Math.max(barW, radius * 2), barHeight, radius);
                context.shadowColor = `${color.replace('rgb', 'rgba').replace(')', ', 0.5)')}`; // Use extracted color for shadow
                context.shadowBlur = 10;
                context.fill();
                context.shadowBlur = 0; // Reset

                // --- Improved Labeling Logic (Left Axis) ---

                // Avatar (Left of axis line)
                ...
                if (imgObj && imgObj.loaded) {
                    context.drawImage(imgObj.img, avatarX, avatarY, imgSize, imgSize);
                } else {
                    context.fillStyle = '#222';
                    context.fillRect(avatarX, avatarY, imgSize, imgSize);
                }
                context.restore();
                ...
                if (nameWidth < maxNameWidth) {
                    ...
                    // Positions tailored to fit above the bar center
                    context.fillText(line1, avatarX - 10, barY - (barHeight * 0.4));
                    context.fillText(line2, avatarX - 10, barY); // Center line
                }

                // Team Name & Logo (Position depends on lines used)
                const teamY = usedLines === 1
                    ? barY + (barHeight * 0.4)
                    : barY + (barHeight * 0.45); // Lower if 2 lines

                // 1. Team Name Text (Right Aligned)
                context.fillStyle = '#aaa';
                context.font = `400 ${barHeight * 0.4}px 'Inter', sans-serif`;
                context.fillText(p.subLabel || '', avatarX - 10, teamY);

                // 2. Team Logo (Left of Team Name)
                if (teamData && teamData.loaded && teamData.img) {
                    ...
                    // Position: avatarX - 10 - textWidth - spacing - logoSize
