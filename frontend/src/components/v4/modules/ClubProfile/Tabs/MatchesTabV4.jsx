...
// --- Helpers ---
const getResult = (m, clubId) => {
    if (m.goals_home == null || m.goals_away == null) return null;
    const isHome = String(m.home_club_id) === String(clubId);
...
const MatchRow = ({ m, clubId, clubName, navigate }) => {
    const isHome = String(m.home_club_id) === String(clubId);
    const result = getResult(m, clubId);
    const [hg, ag] = [Number(m.goals_home), Number(m.goals_away)];

    const homeTeamName = isHome ? clubName : m.home_name;
    const awayTeamName = isHome ? m.away_name : clubName;
...
// --- Main Component ---
const MatchesTabV4 = ({ clubId, season, competitionId, clubName }) => {
    ...
    useEffect(() => {
        ...
        const fetchMatches = async () => {
            ...
            try {
                ...
                const data = await api.getV4ClubMatches(clubId, {
                    season,
        ...
        fetchMatches();
    }, [clubId, season, competitionId]);

    const { finished, scheduled, competitionList } = useMemo(() => {
        let filtered = [...matches];

        if (venueFilter === 'home') {
            filtered = filtered.filter(m => String(m.home_club_id) === String(clubId));
        } else if (venueFilter === 'away') {
            filtered = filtered.filter(m => String(m.away_club_id) === String(clubId));
        }

        if (resultFilter !== 'all') {
            filtered = filtered.filter(m => {
                const r = getResult(m, clubId);
                if (!r) return true;
    ...
    }, [matches, clubId, venueFilter, resultFilter, selectedCompName]);
        ...
        <Stack gap="var(--spacing-xl)">
            ...
            <Grid columns="1fr" gap="var(--spacing-lg)">
                {scheduled.length > 0 && (
                    <Accordion title="Upcoming Fixtures" defaultExpanded={true} maxHeight="750px">
                        <Stack gap="var(--spacing-xs)">
                            {scheduled.map(m => <MatchRow key={m.fixture_id} m={m} clubId={String(clubId)} clubName={clubName} navigate={navigate} />)}
                        </Stack>
                    </Accordion>
                )}
                ...
                <Accordion title="Match History" defaultExpanded={true} maxHeight="750px" headerRight={headerFilters}>
                    <Stack gap="var(--spacing-xs)">
                        {finished.map(m => <MatchRow key={m.fixture_id} m={m} clubId={String(clubId)} clubName={clubName} navigate={navigate} />)}
                        {finished.length === 0 && (
