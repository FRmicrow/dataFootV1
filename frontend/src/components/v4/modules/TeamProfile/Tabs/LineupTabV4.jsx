...
const LineupTab = ({ teamId, year, competitionId, roster }) => {
    ...
    useEffect(() => {
        const fetchLineup = async () => {
            setLoading(true);
            try {
                const data = await api.getV4TypicalLineup(teamId, {
                    year,
    ...
    const formationCoords = useMemo(() => {
        ...
        const rows = parts.map(part => parseInt(part) || 0).filter(n => n > 0);
    ...
    const activeStarters = useMemo(() => {
        ...
        // Sort roster by starts descending for the Starting XI
        const sortedRoster = [...roster].sort((a, b) => (b.starts || 0) - (a.starts || 0));
        ...
        window.DEBUG_STARTERS = newStarters; window.DEBUG_COUNTS = { gk: availGK.length, def: availDEF.length, mid: availMID.length, att: availATT.length, rows: rows }; return newStarters.slice(0, 11);
    }, [lineup, roster]);
            ...
            <Grid columns="300px 1fr" gap="var(--spacing-xl)">
                ...
                <Stack gap="var(--spacing-lg)">
                    ...
                    <Card>
                        ...
                        <div className="sov4-pitch-scroll-wrapper" style={{ overflowX: 'auto', overflowY: 'hidden', paddingBottom: '12px', borderRadius: 'var(--radius-lg)' }}>
                            <div style={{
                                marginTop: '24px',
                                position: 'relative',
