...
from dataset import fetch_cards_dataset

MODEL_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..', 'models', 'cards_total'))

def train_poisson_model(X_train, y_train, X_test, y_test, cat_features, label):
    """
    Train a single CatBoost Poisson Regressor for Cards.
    """
...
def run_training():
    ...
    target_home = 'target_home_cards'
    target_away = 'target_away_cards'
    ...
    avg_home_cards = np.mean(y_test_h)
    avg_away_cards = np.mean(y_test_a)
    print(f"Test Set Average Cards: Home {avg_home_cards:.2f}, Away {avg_away_cards:.2f}")
    ...
    # Feature Importance
    print("\nTop 5 Feature Importance (Home Cards Model):")
    ...
    print("\nTop 5 Feature Importance (Away Cards Model):")
    feature_importances = away_model.get_feature_importance()
    for score, name in sorted(zip(feature_importances, features), reverse=True)[:5]:
        print(f"  {name}: {score:.2f}")
        
    # 5. Save Models
    home_path = os.path.join(MODEL_DIR, f'catboost_cards_v1_home.cbm')
    away_path = os.path.join(MODEL_DIR, f'catboost_cards_v1_away.cbm')
