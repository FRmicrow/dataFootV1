...
from model_paths import get_corners_poisson_paths, with_horizon_suffix
from src.models.model_utils import get_valid_cat_features

from dataset import fetch_corners_dataset, fetch_corners_dataset_v2
...
def run_training(version="v1", horizon_type="FULL_HISTORICAL", activate=True):
    ...
    model_paths = with_horizon_suffix(get_corners_poisson_paths(version), horizon_slug)
    os.makedirs(model_paths["dir"], exist_ok=True)

    if version == "v2":
        df = fetch_corners_dataset_v2().sort_values("match_date")
        features = list(GLOBAL_1X2_FEATURE_COLUMNS)
    else:
        df = fetch_corners_dataset().sort_values("match_date")
        features = [
            "league_id",
    ...
    print(f"Over 9.5 directional accuracy: {over95_acc:.4f}")
    ...
    if version == "v2":
        ...
        metadata_path = os.path.join(model_paths["dir"], f"catboost_corners_v2_{horizon_slug}_metadata.json")
        with open(metadata_path, "w") as handle:
            json.dump(metadata, handle, indent=2)

        importance_path = os.path.join(model_paths["dir"], f"catboost_corners_v2_{horizon_slug}_importance.json")
        with open(importance_path, "w") as handle:
            json.dump(importance.to_dict("records"), handle)

        conn = get_connection()
        try:
            version_tag = f"catboost_corners_{horizon_slug}_v{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            registry_metadata = {
                **metadata,
            ...
            conn.commit()
            print(f"Registered CORNERS_OU model as {version_tag}")
        finally:
            conn.close()
