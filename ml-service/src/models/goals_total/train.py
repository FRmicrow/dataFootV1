...
from model_paths import get_goals_poisson_paths, with_horizon_suffix

from dataset import fetch_goals_dataset
...
def run_training(horizon_type="FULL_HISTORICAL", activate=True):
    horizon_type = normalize_horizon_type(horizon_type)
    horizon_slug = horizon_type.lower()
    model_paths = with_horizon_suffix(get_goals_poisson_paths(), horizon_slug)
    os.makedirs(model_paths["dir"], exist_ok=True)

    df = fetch_goals_dataset().sort_values('match_date')
    ...
    y_train_h = train_df['target_home_goals']
    y_test_h = test_df['target_home_goals']
    y_train_a = train_df['target_away_goals']
    y_test_a = test_df['target_away_goals']

    print(f"\nFeatures used: {len(features)} columns")
    print(f"Train size: {len(train_df)} | Test size: {len(test_df)}")

    home_model, preds_home = train_poisson_model(X_train, y_train_h, X_test, y_test_h, "Home Goals")
    away_model, preds_away = train_poisson_model(X_train, y_train_a, X_test, y_test_a, "Away Goals")
    ...
    over25_acc = float(np.mean(((total_pred > 2.5).astype(int)) == ((total_actual > 2.5).astype(int))))

    print(f"\nTotal Goals Prediction - Overall RMSE: {total_rmse:.4f} Goals")
    print(f"Over 2.5 directional accuracy: {over25_acc:.4f}")
    ...
    metadata = {
        ...
        "over_2_5_accuracy": over25_acc,
        "features_count": len(features),
    }

    metadata_path = os.path.join(model_paths["dir"], f"catboost_goals_v1_{horizon_slug}_metadata.json")
    with open(metadata_path, "w") as handle:
        json.dump(metadata, handle, indent=2)

    importance_path = os.path.join(model_paths["dir"], f"catboost_goals_v1_{horizon_slug}_importance.json")
    with open(importance_path, "w") as handle:
        json.dump(importance.to_dict('records'), handle)

    conn = get_connection()
    try:
        version_tag = f"catboost_goals_{horizon_slug}_v{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        registry_metadata = {
            **metadata,
        ...
        conn.commit()
        print(f"Registered GOALS_OU model as {version_tag}")
    finally:
        conn.close()
...
if __name__ == "__main__":
    ...
    run_training(horizon_type=args.horizon, activate=not args.no_activate)
