...
def load_models(version='v0'):
    ...
    if version not in _MODELS:
        ...
        if not os.path.exists(home_path) or not os.path.exists(away_path):
            raise FileNotFoundError(f"Models for version {version} not found in {os.path.join(BASE_DIR, 'models', 'ht_result')}")
        ...
        _MODELS[version] = (home_model, away_model)
...
def fetch_features_for_inference(fixture_id, include_process=False):
    ...
    # Needs to find home/away team for this fixture
    ...
    # Specify columns to ensure correct order
...
def predict_ht_result(fixture_id, version='v0'):
    ...
    Returns expected goals and a probability matrix for 1N2.
    """
    home_model, away_model = load_models(version)
    
    # 1. Fetch Features
    df = fetch_features_for_inference(fixture_id, include_process=(version=='v1'))
    
    # 2. Predict Expected Goals (HT)
    h_mu = max(0.01, home_model.predict(df)[0])
    a_mu = max(0.01, away_model.predict(df)[0])
    
    # 3. Calculate Poisson Probabilities for Exact Scores
