"""
train_corners_v4.py — Train Poisson regressors for FT corners (home + away) on V4 data.

Data source: v4.ml_feature_store JOIN v4.matches JOIN v4.match_stats
Targets:     home_corners_ft, away_corners_ft (~19k rows, top leagues 2015+)
Model:       CatBoostRegressor with Poisson loss
Output:      models/v4/corners/model_corners_home.cbm + model_corners_away.cbm
"""
...
MODEL_DIR  = os.path.join(BASE_DIR, "models", "v4", "corners")
HOME_PATH  = os.path.join(MODEL_DIR, "model_corners_home.cbm")
AWAY_PATH  = os.path.join(MODEL_DIR, "model_corners_away.cbm")
...
def load_dataset() -> pd.DataFrame:
    ...
    try:
        ...
        cur.execute(
            ...
            WHERE fs.feature_set_id = %s
              AND ms.home_corners_ft IS NOT NULL
              AND ms.away_corners_ft IS NOT NULL
    ...
    logger.info(f"Loaded {len(rows)} corners rows")
    records = []
    for match_id, match_date, corners_h, corners_a, fv_raw in rows:
        try:
            ...
            row["__corners_home__"] = max(0, int(corners_h))
            row["__corners_away__"] = max(0, int(corners_a))
...
def objective(trial, X, y):
    ...
    for tr_idx, val_idx in tscv.split(X):
        ...
        pred = np.clip(m.predict(X.iloc[val_idx]), 0, 30)
...
def train_side(X_train, X_test, y_train, y_test, label, n_trials, no_tune, path):
    ...
    pred = np.clip(model.predict(X_test), 0, 30)
    rmse = np.sqrt(mean_squared_error(y_test, pred))
    mae  = mean_absolute_error(y_test, pred)
    logger.info(f"[Corners {label}] RMSE={rmse:.4f}  MAE={mae:.4f}")
...
def main():
    ...
    if len(df) < 500:
        logger.error(f"Not enough corners data ({len(df)} rows).")
        return
    ...
    metrics = {
        "home": train_side(X_train, X_test, df["__corners_home__"].values[:split],
                           df["__corners_home__"].values[split:], "home", args.trials, args.no_tune, HOME_PATH),
        "away": train_side(X_train, X_test, df["__corners_away__"].values[:split],
                           df["__corners_away__"].values[split:], "away", args.trials, args.no_tune, AWAY_PATH),
    }
