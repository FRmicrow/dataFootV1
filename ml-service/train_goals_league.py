...
LEAGUE_MODEL_ROOT = os.path.join(BASE_DIR, "models", "league_goals_ou")
...
def get_league_model_paths(league_id: int, horizon_slug: Optional[str] = None):
    ...
    return {
        "dir": model_dir,
        "home": os.path.join(model_dir, f"home_goals{suffix}.cbm"),
        "away": os.path.join(model_dir, f"away_goals{suffix}.cbm"),
        "importance": os.path.join(model_dir, f"importance{suffix}.json"),
        "metadata": os.path.join(model_dir, f"metadata{suffix}.json"),
    }


def fetch_goals_league_dataset(league_id: int):
    ...
    try:
        query = """
            SELECT
                ...
                f.goals_home AS target_home_goals,
                f.goals_away AS target_away_goals,
                fs.feature_vector
            FROM V3_Fixtures f
    ...
    if df.empty:
        raise RuntimeError(f"No goals fixtures found for league {league_id}")
...
def train_poisson_model(X_train, y_train, X_test, y_test):
...
def train_league_goals_model(league_id: int, horizon_type: str = "FULL_HISTORICAL", activate: bool = True):
    horizon_type = normalize_horizon_type(horizon_type)
    horizon_slug = horizon_type.lower()
    df = fetch_goals_league_dataset(league_id).sort_values("match_date")
    ...
    home_model, preds_home = train_poisson_model(X_train, y_train_h, X_test, y_test_h)
    away_model, preds_away = train_poisson_model(X_train, y_train_a, X_test, y_test_a)
    ...
    conn = get_connection_db()
    try:
        version_tag = f"league_goals_v{league_id}_{horizon_slug}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        with conn.cursor() as cur:
            if activate:
                cur.execute(
                    ...
                    """,
                    (f"league_goals_ou_{league_id}", "METAMODEL"),
                )
            cur.execute(
                """
...
if __name__ == "__main__":
    ...
    train_league_goals_model(args.league_id, horizon_type=args.horizon, activate=not args.no_activate)
