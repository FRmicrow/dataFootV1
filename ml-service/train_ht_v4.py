"""
train_ht_v4.py — Train Poisson regressors for HT goals (home + away) on V4 data.

Data source: v4.ml_feature_store JOIN v4.matches JOIN v4.match_stats
Targets:     home_score_ht, away_score_ht (~19k rows, top leagues 2015+)
Model:       CatBoostRegressor with Poisson loss
Output:      models/v4/ht_result/model_ht_home.cbm + model_ht_away.cbm

Usage:
    python train_ht_v4.py [--trials N] [--no-tune]
"""
...
MODEL_DIR  = os.path.join(BASE_DIR, "models", "v4", "ht_result")
HOME_PATH  = os.path.join(MODEL_DIR, "model_ht_home.cbm")
AWAY_PATH  = os.path.join(MODEL_DIR, "model_ht_away.cbm")
...
def load_dataset() -> pd.DataFrame:
    ...
    try:
        ...
        cur.execute(
            ...
            SELECT fs.match_id, m.match_date,
                   ms.home_score_ht, ms.away_score_ht,
                   fs.feature_vector
            ...
            WHERE fs.feature_set_id = %s
              AND ms.home_score_ht IS NOT NULL
              AND ms.away_score_ht IS NOT NULL
    ...
    logger.info(f"Loaded {len(rows)} HT rows (top leagues only)")
    records = []
    for match_id, match_date, ht_home, ht_away, fv_raw in rows:
        try:
            ...
            row["__ht_home__"]     = int(ht_home)
            row["__ht_away__"]     = int(ht_away)
...
def objective(trial, X, y):
    ...
    for tr_idx, val_idx in tscv.split(X):
        ...
        pred = np.clip(m.predict(X.iloc[val_idx]), 0, 10)
...
def train_side(X_train, X_test, y_train, y_test, label, n_trials, no_tune):
    ...
    logger.info(f"[HT {label}] RMSE={rmse:.4f}  MAE={mae:.4f}")
...
def main():
    ...
    if len(df) < 500:
        logger.error(f"Not enough HT data ({len(df)} rows). Need at least 500.")
        return
    ...
    metrics = {}
    for target, label, path in [
        ("__ht_home__", "home", HOME_PATH),
        ("__ht_away__", "away", AWAY_PATH),
    ]:
        model, m = train_side(
            X_train, X_test,
            df[target].values[:split], df[target].values[split:],
            label, args.trials, args.no_tune
        )
