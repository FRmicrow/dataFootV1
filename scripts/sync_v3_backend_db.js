...
// Paths - Targeted at backend/ where the actual data resides
const sourceDbPath = path.resolve(__dirname, '../backend/database.sqlite');
const targetDbPath = path.resolve(__dirname, '../backend/database_v3_test.sqlite');

console.log('🚀 Starting targeted cross-database sync to backend/database_v3_test.sqlite...');
...
async function main() {
    try {
        ...
        console.log(`📊 Result: ${insertedCount} inserted, ${updatedCount} updated into backend/database_v3_test.sqlite.`);

    } catch (error) {
        console.error('❌ Error during targeted sync:', error);
    } finally {
        sourceDb.close();
        targetDb.close();
    }
